...
class SearchEngine {
  ...
  findClosestWord(word: string) {
    ...
    for (const dictWord in Object.keys(this.invertedIndex)) {
  ...
  search(target_text: string) {
    ...
    target_tokens.forEach((token) => {
      ...
      target_posts.forEach((post_id) => {
        ...
        if (full_post.title.includes(token)) score_board[post_id] += 4;
        if (full_post.object.includes(token)) score_board[post_id] += 3;
        if (full_post.tags.some((tag) => tag.toLowerCase().includes(token)))
          score_board[post_id] += 2;
        if (full_post.user_data.name.includes(token)) score_board[post_id] += 2;
      });
    });
